...
for identifier in   NDAR_INVJD453TLY NDAR_INVTM934PYA NDAR_INVDJ468TMT NDAR_INVFL083XJW NDAR_INVAB260JTF NDAR_INVXF831YC7 NDAR_INVMJ663MA3 NDAR_INVRG377UU4 NDAR_INVRY400UV1 NDAR_INVLE676HGB NDAR_INVLL129TDJ NDAR_INVUH778BGU NDAR_INVRM149GDG NDAR_INVLK752PXM NDAR_INVCX823LF1 NDAR_INVDX592UET NDAR_INVGH681TMN   #NDAR_INVFC157TK5 
#`cat temp.A2.R.txt`
...
####
	   ...
	   for region2 in   Matrix-like_Ctx_${group} # Striosome-like_Ctx_${group}
              do
...
#    line is removed. If it's not removed, then at the end of the script this file will be read out as subjects needing manual attention.
           ...
           threshold_volume=`fslstats ${identifier}_CT_LC_${hemisphere1}-${region1}_to_Striosome-like_Ctx_${group}_excludesMidline_proj_seg_thr_1.nii.gz -l 0.55 -V | awk '{print $1}' `
           mv ${identifier}_CT_LC_${hemisphere1}-${region1}_to_Striosome-like_Ctx_${group}_excludesMidline_PT1_1.5SDthresh.nii.gz ../
...
echo " "
		    ...
		    echo "${identifier} ${hemisphere1}-${region1}_to_${region2} yielded a mask of ${target_volume}, ${group}." >> ${experimental_directory}/1.5SDthresh_runs_that_are_too_big.${group}                    
                    break 5
	         fi
              done
           done
...
for denominator in 1.25 1.5 1.75 2 2.5 3 4 5 6 8 12 14 20 25 40 45 60 65 80 85 90 95 100 115 120 125 130 135 140 145 150 160 170 180 190 200 220 250 275 300 325 350 375 400 600 625 900 950 1500 1750 3000 3100 4500 5000 5500 7000 9000 12000 16000 20000 24000 30000 35000 40000 45000 50000 55000 65000 75000 90000 105000 135000 155000 175000 185000 210000 230000 250000 270000 290000 330000 11000 12000 50000 54000 175000 1850000 70000 80000 120000 130000 300000 305000 310000 400000 405000 410000 2000 2100 2200 8000 8100 17000 18000 27000 27500 28000 33000 34000 35000 37000 39000 41000 44000 40000 50000 60000 62000 64000 60 61 62 250 255 260 7500 7600 7700 15000 15200 55000 65000 40000 95000 90000 85000 50000 205000 210000 225000
   do
