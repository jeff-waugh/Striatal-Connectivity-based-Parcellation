...
for identifier in `awk '{print $1}' temp10cA2R.txt`
   do
   ...
   for hemisphere1 in Left  #Right
      ...
      for region1 in trStriatum		
         ...
         for group in 2020.N-1.A2
	    do

######creates ispilateral mask
fslmaths ${directory}.bedpostX/nodif_brain_mask.nii.gz -mas ${directory}.bedpostX/${identifier}_MNI152_${hemisphere1}_hemisphere_block_mask_bin0.5.nii.gz ${directory}.bedpostX/nodif_brain_mask.${hemisphere1}-hemisphere.nii.gz
	       ...
	       twoROImask=${hemisphere1}-${region1}_StrioVsMat_${group}_CT_LC.ipsi
		...
		rm ${twoROImask}/targets.txt ${directory}.bedpostX/${twoROImask}/temp2.sh ${directory}.bedpostX/${twoROImask}/temp3.sh
		  ...
		  Matrix_2=`cat /project/radiology/ANSIR_lab/shared/s188839_workspace/Absolute/Striatal_Parcellation/StriosomeVsMatrix.${group}.txt | awk '{print $1}' | sed -n 2p`
 		  ...
 		  Matrix_5=`cat /project/radiology/ANSIR_lab/shared/s188839_workspace/Absolute/Striatal_Parcellation/StriosomeVsMatrix.${group}.txt | awk '{print $1}' | sed -n 5p`
...
#
		...
		do
		  ...
		  echo "applywarp --ref=${experimental_directory}/FMRIB58_FA_1mm.nii.gz --in=${identifier}_CT_LC_${hemisphere1}-${region1}_to_${region2}_excludesMidline_proj_seg_thr_1.nii.gz --warp=${directory}/${identifier}_transtostandard.mat.nii.gz --out=${identifier}_CT_LC_toMNI_${hemisphere1}-${region1}_to_${region2}_excludesMidline_PT1.nii.gz" >> ${directory}.bedpostX/${twoROImask}/temp3.sh
		  echo "cp ${identifier}_CT_LC_toMNI_${hemisphere1}-${region1}_to_${region2}_excludesMidline_PT1.nii.gz ${experimental_directory}/ForRandomise/${twoROImask}" >> ${directory}.bedpostX/${twoROImask}/temp3.sh
		done
...
#
		 ...
		 echo "probtrackx2 --seed=${seed} -l --loopcheck --onewaycondition -c 0.2 -S 2000 --steplength=0.5 -P 5000 --fibthresh=0.01 --distthresh=0.0 --sampvox=0.0 --forcedir --opd --samples=merged --mask=nodif_brain_mask.${hemisphere1}-hemisphere.nii.gz --dir=${twoROImask}/ --targetmasks=${twoROImask}/targets3.txt --os2t --pd --out=${identifier}_ptracx_${twoROImask}.nii.gz" >> ${directory}.bedpostX/temp1

		 mv ${directory}.bedpostX/temp1 ${directory}.bedpostX/temp_ptracx_${identifier}_${twoROImask}.sh
...
sh ${directory}.bedpostX/temp_ptracx_${identifier}_${twoROImask}.sh &
