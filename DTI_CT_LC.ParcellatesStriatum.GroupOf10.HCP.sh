...
for identifier in `awk '{print $2}' temp.HCPpeds_without20yo.txt`

#`cat temp3d.txt`
#$(cat /project/radiology/ANSIR_lab/shared/jwaugh_workspace/HCP/subjectlist.txt)
   do
...
#
      ...
      echo "probtrackx2 --seed=${seed} -l --loopcheck --onewaycondition -c 0.2 -S 2000 --steplength=0.5 -P 5000 --fibthresh=0.01 --distthresh=0.0 --sampvox=0.0 --forcedir --opd --samples=merged --avoid=${identifier}_MidlinePlane.FA_bin0.5.nii.gz --mask=nodif_brain_mask.nii.gz --dir=${twoROImask}/ --targetmasks=${twoROImask}/targets.txt --os2t --pd --out=${identifier}_ptracx_${twoROImask}.nii.gz" >> ${directory}.bedpostX/temp1
